...
Updated: September 20, 2026. This is the current status and restart point. Public usage lives in [README](../README.md), design in [architecture](../docs/architecture.md), and historical test evidence in [verification](verification.md). Local `artifacts/` results are gitignored and may be absent in another checkout.

## Current handoff

- **F13 complete: `@likith99/react-native-adaptive-liquid-glass@0.1.0` is published and public on the npm registry** (September 20, 2026), from commit `2cbf5f8` with tag `v0.1.0` pushed. The registry tarball was re-downloaded and hashes identically to the locally verified artifact — sha256 `bd027edf…5541`, shasum `c300239f…f8eb`, 77 files — so the published bytes are the ones the standalone consumer built. `npm access get status` reports `public`. [Release record](../artifacts/release-0.1.0.json), [scope record](../artifacts/scope-verification.json). Native identifiers and behavior are unchanged. Remaining physical-device/performance gaps are unaffected by publication and stay open.
- Publishing mechanics worth reusing, all learned the hard way on this release: npm rejects a plain login token with `403 ... Two-factor authentication or granular access token with bypass 2fa enabled is required`, so the account now has 2FA at `auth-and-writes`. npm no longer enrols TOTP authenticators — 2FA is a WebAuthn security key/passkey, which yields no 6-digit code, so **never pass `--otp`**; npm prints an `npmjs.com/auth/cli/<uuid>` URL to approve in the browser. That challenge needs a TTY, or npm exits `EOTP` without waiting. Publishing a packed tarball needs a leading `./`, else npm parses it as a git spec. A first publish can return `PUT 200` while anonymous `GET` 404s for a minute or two; that is read-path propagation, not failure. Full procedure in [releasing](../docs/releasing.md).

- Repository presentation, September 20: `docs/` now holds only user documentation and carries an index; the tracker, verification history, performance review and research notes moved here to `.agent/` with an index of their own, and all 75 relative links were re-checked after the move. `AGENTS.md`, both READMEs and every doc cross-reference point at the new paths. The README leads with `docs/images/demo.gif` (12s, 861KB, recorded on iOS 26.5 by driving `testNativeEventsLivePropsAndFallback` under `simctl io recordVideo`) plus stills for glass merging on/off and the flat and grouped menus. Source captures stay in gitignored `artifacts/`; the committed copies are downscaled.
- Not yet done for npm: `packages/liquid-glass/README.md` still has no images. npmjs.com does not resolve relative paths, so it needs absolute raw.githubusercontent URLs, and it only reaches the registry with the next published version.
- Open from the Socket review: the peer range `>=0.81.0 <0.87.0` excludes React Native 0.87.x, which is current, so a new RN app hits ERESOLVE. Verify on 0.87 and widen in 0.1.1 rather than widening on assumption. The package itself has zero dependencies, zero install scripts and adds zero packages to a consumer tree; `npm audit` is clean.
...
- Existing Claude changes retained: MIT files/metadata, RN peer range `>=0.81.0 <0.87.0`, iOS floor **15.1**, menu availability guards, deployment-target tab reconciliation, and example scene lifecycle. 0.1.0 is MIT licensed and is now publicly released.
...
- Immediate next action now that 0.1.0 is out: profile Release builds on representative physical devices, exercise Reduce Transparency/Motion transitions and broader device/accessibility gaps, then verify distribution builds and refresh the scoped consumer before cutting 0.1.1/0.2.0. Published name/version pairs cannot be reused, so any fix ships as a new version rather than a republish.
...
| F07 | GlassMenuButton | Implemented; native menus on both platforms | Flat-menu regression passed again in B1. Earlier physical-iPhone result covers the flat revision. [API](../docs/menus.md), [earlier record](../artifacts/menu-verification.json). Standalone consumer now includes menus, toolbar, and tabs; see B2 package record. |
| F08 | Native toolbar | Implemented — B1 | Native builds and iOS/Android action, overflow, narrow layout, disabled, replacement, and open-menu dismissal checks passed. [API](../docs/toolbars.md), [B1 record](../artifacts/b1-verification.json). Physical acceptance/shared-background visual comparison pending. |
...
| F10 | Native tab bar | Implemented — B2 | Native builds and iOS/Android selection, disabled activation, badges, reorder/replacement, and remount checks passed. [API](../docs/tabs.md). B3 reviewed disabled-tab reporting: UIKit has no public API for it, so a disabled iOS tab still reports itself enabled. Android reports it. |
...
| F14 | Older iOS support and backward compatibility | In progress — B4 | Floor is 15.1; deployment-target reconciliation is implemented. The 18.6 runtime exercises fallbacks; blur/menu/accessibility checks pass on 18.6; older-device acceptance remains B4. [Support matrix](../docs/compatibility.md). |
...
| B4 — older iOS support | F14: lower the floor to iOS 15.1, run the pre-26 fallback path, recheck menu dismissal, accept on a physical older device | Open. Scope and constraints in [compatibility](../docs/compatibility.md). Blocks the first publish, because the shipped floor decides which devices a consuming app can still reach. |
...
- Published tarball: `artifacts/likith99-react-native-adaptive-liquid-glass-0.1.0.tgz`, sha256 `bd027edf…5541`, identical to the registry copy of 0.1.0. Compare its hash against `package-smoke.json` before reusing evidence. Older unscoped archives are historical.
